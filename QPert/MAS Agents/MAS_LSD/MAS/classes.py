...
class RetrievalAgent:
    ...
    def oredered_distance_list(self, data_set):
        distances = []
        norm_q = np.linalg.norm(self.query)
        for element in data_set:
            norm = np.linalg.norm(element[1])
            normal_ele = element[1] / norm
            dist = np.linalg.norm(norm_q - normal_ele)
            distances.append((element[1], dist, element[2]))
...
class CoordinateAgent:
    ...
    def CrosseOver_Mutation(self):
        Data = self.Clones
        lenght = Data.shape[0]
        new_gen = np.zeros((self.N, 2048, 3))
...
def lsd_discriptors_maker(link , y):
    disc = np.load(link,allow_pickle=True)
    return np.c_[disc, y]
